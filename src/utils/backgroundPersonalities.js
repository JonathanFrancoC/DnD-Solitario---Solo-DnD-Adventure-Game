export const backgroundPersonalities = {
  "acolito": {
    personalityTraits: [
      "Hablo con solemnidad y midiendo cada palabra.",
      "Soy paciente incluso cuando otros pierden la calma.",
      "Procuro ver la chispa de bondad en cualquiera.",
      "Respeto profundamente la jerarquía religiosa.",
      "Busco señales y presagios en lo cotidiano.",
      "Siempre tengo una parábola lista para ilustrar algo.",
      "Tolero otras creencias, pero defiendo la mía con firmeza.",
      "Me esfuerzo por ser un ejemplo de mi fe."
    ],
    ideals: [
      "Tradición. Las costumbres sagradas nos sostienen.",
      "Caridad. Ayudar a los necesitados es una obligación.",
      "Cambio. El mundo mejora con pequeños actos constantes.",
      "Poder. Aspiro a guiar a mi congregación algún día.",
      "Fe. Confío en la guía divina por encima de todo.",
      "Aspiración. Debo ser digno del favor de mi deidad."
    ],
    bonds: [
      "Mi templo es mi hogar y mi deber.",
      "Protejo a mis compañeros de orden a cualquier costo.",
      "Debo la vida a un milagro; vivo para retribuirlo.",
      "Guardo un texto sagrado que no debe perderse.",
      "Prometí llevar consuelo a quien sufre.",
      "Un anciano sacerdote me formó; honro su legado."
    ],
    flaws: [
      "Me cuesta aceptar otras doctrinas.",
      "Confío demasiado en los de mi fe.",
      "Soy inflexible cuando creo tener razón moral.",
      "Desconfío de los extraños por instinto.",
      "Me aferro a planes “por voluntad divina” aunque sean imprudentes.",
      "Me culpo en exceso por mis errores."
    ]
  },

  "charlatan": {
    personalityTraits: [
      "Tengo un alias para cada ocasión.",
      "Improviso historias convincentes con facilidad.",
      "Encuentro placer en engaños ingeniosos.",
      "Me visto y actúo como exija el papel.",
      "Siempre tengo una coartada lista.",
      "Puedo halagar a cualquiera para desarmarlo.",
      "Soy rápido con los dedos y con la lengua.",
      "Río primero y admito después… si me conviene."
    ],
    ideals: [
      "Independencia. Nadie me manda.",
      "Justicia. Engaño a quienes se aprovechan de otros.",
      "Caridad. A veces robo para alimentar bocas hambrientas.",
      "Creatividad. Un buen timo es una obra de arte.",
      "Lealtad. Jamás delato a mis cómplices.",
      "Ambición. Aspiro a un golpe legendario."
    ],
    bonds: [
      "Un viejo estafador me enseñó el oficio.",
      "Guardo un objeto falso que vale más por la historia que por el oro.",
      "Estoy en deuda con alguien peligroso.",
      "Protejo a un aprendiz que ve en mí a un mentor.",
      "Una víctima inocente me recuerda mis límites.",
      "Tengo una “marca” favorita a la que siempre regreso."
    ],
    flaws: [
      "No resisto una oportunidad brillante… aunque sea tonta.",
      "Me creo demasiado listo para ser atrapado.",
      "El juego me pierde.",
      "Cambio de identidad tan seguido que olvido quién soy.",
      "Me cuesta decir la verdad incluso cuando conviene.",
      "Prometo más de lo que puedo cumplir."
    ]
  },

  "criminal": {
    personalityTraits: [
      "Siempre tengo un plan de escape.",
      "Mantengo la compostura bajo presión.",
      "Confío más en contactos que en autoridades.",
      "Prefiero las sombras a los focos.",
      "Evalúo una habitación buscando vías y amenazas.",
      "Hablo poco y observo mucho.",
      "Pongo precio a casi todo.",
      "Me gusta el humor ácido en momentos tensos."
    ],
    ideals: [
      "Honor. No traiciono a quien comparte mi oficio.",
      "Libertad. Romper cadenas es un deber.",
      "Compasión. Los débiles necesitan alguien duro a su lado.",
      "Justicia. Ojo por ojo con los abusadores.",
      "Cambio. Una ciudad se renueva desde sus bajos fondos.",
      "Aspiración. Lideraré mi propio gremio."
    ],
    bonds: [
      "Mi banda es mi familia.",
      "Guardo una lista de favores por cobrar.",
      "Debo proteger a quien me salvó de la horca.",
      "Un barrio me considera su guardián en la sombra.",
      "Jamás abandono a un compañero en el trabajo.",
      "Conservo una insignia de un caso sin resolver."
    ],
    flaws: [
      "Lo brillante me distrae demasiado.",
      "Golpeo primero, pregunto después.",
      "Improviso y “olvido” los planes.",
      "Tengo un problema profundo con la autoridad.",
      "Confío demasiado en mi reputación.",
      "Subestimo los riesgos cuando huelo ganancias."
    ]
  },

  "artista": { // Entertainer
    personalityTraits: [
      "Siempre busco el centro del escenario.",
      "Tengo un truco listo para romper el hielo.",
      "Recuerdo rostros del público años después.",
      "Practico hasta que el número sale perfecto.",
      "Colecciono historias y canciones de cada ciudad.",
      "No soporto los silencios incómodos.",
      "Adoro los aplausos tanto como el aire.",
      "Siempre sé a quién dedicar una balada."
    ],
    ideals: [
      "Belleza. El arte puede cambiar corazones.",
      "Libertad. La expresión no admite cadenas.",
      "Tradición. Mantengo vivo el folclore.",
      "Cambio. Cada función debe sorprender.",
      "Gloria. Mi nombre resonará en tabernas lejanas.",
      "Comunidad. Actúo para levantar el ánimo colectivo."
    ],
    bonds: [
      "Debo todo a mi maestro de escena.",
      "Guardo un instrumento con valor sentimental.",
      "Prometí actuar en honor a alguien perdido.",
      "Un público fiel me espera siempre en una taberna.",
      "Un rival me impulsa a superarme.",
      "Tengo letras escritas para una sola persona."
    ],
    flaws: [
      "Hambriento de atención, interrumpo sin querer.",
      "Vivo por encima de mis posibilidades.",
      "Soy vanidoso con mis talentos.",
      "Me deprimo si no aplauden lo suficiente.",
      "Coqueteo incluso cuando no debo.",
      "Me cuesta aceptar críticas."
    ]
  },

  "heroe": { // Héroe del pueblo (Folk Hero)
    personalityTraits: [
      "Ayudo sin pedir nada a cambio.",
      "Trabajo duro y espero lo mismo.",
      "No abandono a nadie atrás.",
      "Enfrento injusticias aunque cueste caro.",
      "Tengo un dicho campesino para cada ocasión.",
      "No presumo; dejo que hablen mis actos.",
      "Escucho primero, actúo después.",
      "Conozco a todos en el pueblo por su nombre."
    ],
    ideals: [
      "Respeto. Toda persona merece dignidad.",
      "Justicia. Lo correcto por encima de lo fácil.",
      "Libertad. La tiranía no echa raíces con gente valiente.",
      "Comunidad. Mi gente es mi fuerza.",
      "Destino. Estoy llamado a proteger a los míos.",
      "Gloria. Las hazañas inspiran a futuros héroes."
    ],
    bonds: [
      "Defiendo mi aldea cueste lo que cueste.",
      "Conservo la herramienta de un antepasado.",
      "Un niño me ve como su modelo a seguir.",
      "Le debo la vida a un veterano local.",
      "Una promesa hecha en tiempos duros me guía.",
      "Una vieja herida me recuerda por qué lucho."
    ],
    flaws: [
      "Acepto retos imprudentes.",
      "Confío demasiado en la buena fe ajena.",
      "Me sacrifico de más por otros.",
      "Soy testarudo cuando creo tener razón.",
      "Minimizo a los poderosos por orgullo.",
      "Me cuesta pedir ayuda."
    ]
  },

  "artesano": { // Guild Artisan
    personalityTraits: [
      "Valoro el trabajo bien hecho por encima del aplauso.",
      "Anoto cada pedido con meticulosidad.",
      "Huelo los materiales para evaluar su calidad.",
      "Regateo con una sonrisa… y no cedo fácil.",
      "Guardo muestras de todo lo que elaboro.",
      "Me gusta enseñar mi oficio a aprendices.",
      "Soy puntilloso con los plazos.",
      "Trato a clientes y proveedores con cortesía firme."
    ],
    ideals: [
      "Tradición. El oficio honra a mis mayores.",
      "Trabajo. La excelencia se forja día a día.",
      "Comunidad. El gremio eleva la ciudad.",
      "Innovación. Siempre hay una mejor manera.",
      "Independencia. Mi taller, mis reglas.",
      "Prosperidad. La riqueza bien ganada es virtud."
    ],
    bonds: [
      "Mi gremio es mi sostén y mi familia.",
      "Un cliente fiel me salvó de la ruina.",
      "Herede un taller que debo mantener vivo.",
      "Un aprendiz depende de mi guía.",
      "Un encargo prestigioso puede consagrarme.",
      "Protejo un diseño secreto del taller."
    ],
    flaws: [
      "Soy terco con mis métodos.",
      "Desprecio el trabajo descuidado… quizá demasiado.",
      "Acumulo materiales “por si acaso”.",
      "Me obsesiono con encargos importantes.",
      "Me ofendo si regatean de más.",
      "Prometo plazos imposibles por ambición."
    ]
  },

  "ermitaño": {
    personalityTraits: [
      "Prefiero escuchar al murmullo del viento.",
      "Interpreto coincidencias como señales.",
      "Me incomodan las multitudes.",
      "Hablo en metáforas sin darme cuenta.",
      "Encuentro belleza en lo austero.",
      "Desconfío de lo innecesario.",
      "Tengo una rutina diaria casi sagrada.",
      "La soledad me enseñó a conocerme."
    ],
    ideals: [
      "Autoconocimiento. Mirar adentro revela verdades.",
      "Misterio. Hay secretos que merecen respeto.",
      "Soledad. El silencio nutre el espíritu.",
      "Iluminación. Busco claridad para todos.",
      "Compartir. Las revelaciones no son solo mías.",
      "Renovación. Lo viejo muere para que nazca lo nuevo."
    ],
    bonds: [
      "Guardo un manuscrito de gran valor.",
      "Un lugar remoto es mi santuario.",
      "Prometí llevar un mensaje cuando llegara el momento.",
      "Un maestro espiritual me guía en sueños.",
      "Un objeto humilde me ancla al mundo.",
      "Debo proteger un secreto peligroso."
    ],
    flaws: [
      "Soy excéntrico y me cuesta disimularlo.",
      "Creo entender mejor a todos… y lo hago notar.",
      "Evito conflictos hasta que es tarde.",
      "Me aferro a visiones discutibles.",
      "Divulgo detalles sensibles por ingenuidad.",
      "Pierdo la noción del tiempo con facilidad."
    ]
  },

  "noble": {
    personalityTraits: [
      "Mi porte refleja mi linaje.",
      "Hablo con diplomacia incluso al discrepar.",
      "Recuerdo nombres y cortesías al detalle.",
      "Valoro el decoro en público.",
      "Tengo una anécdota familiar para cada ocasión.",
      "No tolero la grosería gratuita.",
      "Delego, pero superviso todo.",
      "Sé cuándo un banquete es política disfrazada."
    ],
    ideals: [
      "Respeto. Toda persona merece trato digno.",
      "Responsabilidad. La posición implica deberes.",
      "Independencia. Probaré estar a la altura sin privilegios.",
      "Poder. Influir para cambiar el mundo.",
      "Familia. La casa es lo primero.",
      "Obligación. Debo proteger al pueblo."
    ],
    bonds: [
      "Defiendo el buen nombre de mi casa.",
      "Guardo un legado que no debe perderse.",
      "Protejo a un vasallo leal.",
      "Mi escudero confía en mí ciegamente.",
      "Un acuerdo político me ata a otra familia.",
      "Una promesa sellada en la infancia me guía."
    ],
    flaws: [
      "Me creo el centro del salón.",
      "Me ofendo con facilidad ante faltas de etiqueta.",
      "Subestimo a la gente común.",
      "No olvido un desaire.",
      "Puedo ser condescendiente sin querer.",
      "Me cuesta admitir errores en público."
    ]
  },

  "forastero": { // Outlander
    personalityTraits: [
      "Me siento más libre bajo el cielo abierto.",
      "Hablo poco y voy al grano.",
      "Leo huellas y rastros como otros leen libros.",
      "Me oriento por estrellas y viento.",
      "Desconfío de la pompa urbana.",
      "Comparto sin dudar lo que tengo.",
      "Conozco plantas y sus usos.",
      "Escucho antes a los animales que a la gente."
    ],
    ideals: [
      "Libertad. Nadie posee la tierra.",
      "Naturaleza. Vivimos con el mundo, no sobre él.",
      "Supervivencia. Haré lo necesario para seguir.",
      "Comunidad. Mi clan y mi manada primero.",
      "Equilibrio. Todo toma y devuelve.",
      "Destino. Los presagios marcan el camino."
    ],
    bonds: [
      "Mi tribu me necesita.",
      "Un valle sagrado debe permanecer intacto.",
      "Un animal me salvó la vida y lo honro.",
      "Llevo un tótem familiar conmigo.",
      "Guío viajeros fuera de peligro.",
      "Debo saldar una deuda con otro clan."
    ],
    flaws: [
      "Desconfío de casi todos al principio.",
      "Soy brusco y hiero sin querer.",
      "Me enfado con los desperdicios.",
      "Guardo rencor largo tiempo.",
      "Me pierdo en supersticiones de mi pueblo.",
      "Me cuesta pedir indicaciones… incluso perdido."
    ]
  },

  "erudito": { // Sage
    personalityTraits: [
      "Cito fuentes incluso en la taberna.",
      "Me pierdo horas entre libros.",
      "Formulo preguntas hasta saciar la curiosidad.",
      "Hablo con precisión pedante.",
      "Guardo notas en todos mis bolsillos.",
      "Valoro más una verdad que mil halagos.",
      "Observo patrones donde otros no ven nada.",
      "Disfruto enseñando lo aprendido."
    ],
    ideals: [
      "Conocimiento. Saber abre todas las puertas.",
      "Lógica. La razón por encima del ímpetu.",
      "Libre Pensamiento. Nadie monopoliza la verdad.",
      "Poder. El saber es influencia.",
      "Compartir. El conocimiento crece al repartirse.",
      "Autosuficiencia. Aprender para depender menos."
    ],
    bonds: [
      "Un mentor marcó mi camino.",
      "Busco un tomo perdido legendario.",
      "Mi biblioteca es mi tesoro.",
      "Debo concluir una investigación larga.",
      "Un colega rival me inspira y provoca.",
      "Prometí educar a mentes jóvenes."
    ],
    flaws: [
      "Hablo en exceso cuando me apasiono.",
      "Menosprecio el conocimiento práctico.",
      "Me distraigo con facilidad por curiosidad.",
      "Olvido asuntos mundanos por estudiar.",
      "Me cuesta admitir lagunas en mi saber.",
      "Arriesgo de más por una pista jugosa."
    ]
  },

  "marinero": {
    personalityTraits: [
      "Cuento nudos como otros cuentan monedas.",
      "Conozco historias de cada puerto.",
      "Silbo para llamar al viento… superstición o no.",
      "Mantengo la calma en tormentas y broncas.",
      "Hago amigos en cualquier muelle.",
      "Reparo lo que sea con lo que haya.",
      "Hablo en jerga marinera sin darme cuenta.",
      "Duermo mejor con el balanceo del mar."
    ],
    ideals: [
      "Libertad. El mar no tiene amo.",
      "Lealtad. La tripulación antes que nada.",
      "Supervivencia. Reglas hechas en sal y sangre.",
      "Exploración. Siempre hay otra costa.",
      "Disciplina. La nave es orden o muerte.",
      "Destino. Las mareas me guían."
    ],
    bonds: [
      "Debo volver a un puerto donde me esperan.",
      "Guardo el recuerdo de un capitán caído.",
      "Tengo una deuda con un contramaestre.",
      "Un navío es mi verdadero hogar.",
      "Busco un tesoro marcado en un mapa viejo.",
      "Prometí llevar un mensaje a través del océano."
    ],
    flaws: [
      "Bebo más de la cuenta en tierra.",
      "Apuesto mi paga con facilidad.",
      "Provoco peleas de taberna por deporte.",
      "Subestimo los peligros del clima.",
      "Me cuesta obedecer a capitanes nefastos.",
      "Soy rencoroso con oficiales injustos."
    ]
  },

  "soldado": {
    personalityTraits: [
      "Mantengo el equipo impecable.",
      "Mido el terreno instintivamente.",
      "Sigo órdenes… pero no soy ciego.",
      "Conozco el valor del silencio táctico.",
      "Animo a los demás en momentos duros.",
      "Me levanto antes del alba por costumbre.",
      "No dejo preguntas sin respuesta en el informe.",
      "Respeto rangos aunque no comparta decisiones."
    ],
    ideals: [
      "Mayor Bien. Lucho por algo más grande que yo.",
      "Responsabilidad. Cumplir órdenes salva vidas.",
      "Independencia. La obediencia también piensa.",
      "Fuerza. La preparación vence a la suerte.",
      "Lealtad. Mi unidad es mi familia.",
      "Honor. Mi palabra pesa como acero."
    ],
    bonds: [
      "Mi escuadrón me necesita.",
      "Guardo un estandarte de campaña.",
      "Debo a un camarada la vida.",
      "Vigilo a la familia de un caído.",
      "Defiendo una frontera que pocos miran.",
      "Una promesa hecha en el frente me guía."
    ],
    flaws: [
      "Me cuesta dejar la disciplina… incluso en fiesta.",
      "Soy impaciente con la incompetencia.",
      "Subestimo a civiles en combate.",
      "Guardo rencores de guerra.",
      "Me vuelvo temerario para proteger a otros.",
      "Obedezco incluso cuando debería cuestionar."
    ]
  },

  "huerfano": { // Urchin
    personalityTraits: [
      "Sé colarme donde otros ni miran.",
      "Estoy atento a cada sombra y susurro.",
      "Comparto comida con quien la necesita.",
      "Conozco los atajos de cualquier barrio.",
      "No confío rápido, pero soy leal al confiar.",
      "Aprendí a hacer reír para sobrevivir.",
      "Valoro objetos pequeños con grandes historias.",
      "Tengo manos ligeras y pies más ligeros aún."
    ],
    ideals: [
      "Cambio. Nadie está condenado a su cuna.",
      "Comunidad. La calle cuida a la calle.",
      "Libertad. No vuelvo a vivir bajo un amo.",
      "Justicia. Los poderosos deben mirar abajo.",
      "Esperanza. Siempre hay una salida.",
      "Astucia. Se sobrevive pensando rápido."
    ],
    bonds: [
      "Debo proteger a niños como fui yo.",
      "Una figura amable me dio una segunda oportunidad.",
      "Un escondite secreto guarda mis recuerdos.",
      "Tengo un hermano perdido que debo encontrar.",
      "Un perro callejero es mi mejor amigo.",
      "Le prometí a alguien dejar la delincuencia."
    ],
    flaws: [
      "Desconfío incluso de los amigos.",
      "Tiendo a robar cuando me asusto.",
      "Reacciono mal a voces y órdenes.",
      "Me cuesta aceptar autoridad legítima.",
      "Me aferro a cosas inútiles por miedo a perder.",
      "Huyo cuando debería plantar cara."
    ]
  }
};
